...
export function applyOpenInferenceInstrumentationName(span: ReadableSpan): void {
    ...
    (
        span.instrumentationLibrary as { name: string }
    ).name = `openinference.instrumentation.${providerName}`;
}
